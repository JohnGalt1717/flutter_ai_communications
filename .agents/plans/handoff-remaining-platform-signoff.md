...
- Windows exclusive audio 20-cycles: extra desktop evidence (PR #32 comment
  on #26). Linux/WSLg 20-cycles ran in the #33 window (Plantronics/WASAPI on
  Windows; Pulse `RDPSource`/`RDPSink` on WSL).
- Windows camera: LifeCam Studio `native_camera_test` passed in #34.
- Linux camera: V4L2 graph in tree; `flutter build linux` + physical receipt
  remaining. Camera work is tracked in
  `.agents/plans/video-capture-and-sinks.md`, not this audio handoff.
- Mac USB capture RMS=0 leftover below was **not re-run** on `e6b37b4`.
  Treat it as still the Mac product bug until a new exclusive Orchestration
  receipt with USB attached proves otherwise.
...
| Chrome | `chrome` | `…-web-chrome-cycles.json` + `…-web-chrome.json` at `10f9c2c` | 20-cycles + 45 capture×render on #21. `devicechange` still `skipped=capability`. Not re-run on `e6b37b4`. |
| Windows | `windows` | audio 20-cycles (PR #32/#33 window); camera LifeCam Studio (`e6b37b4`) | Audio extra desktop evidence. Camera receipt is video-track, not #26 close. |
| Linux | `linux` | audio 20-cycles on WSLg (PR #33 window) | Pulse `RDPSource`/`RDPSink`. Camera graph in tree; Linux camera receipt remaining. |
...
Adapter now emits Observed after successful `startNative` and after `selectEndpoints` when running. Unit test “start and select report Observed from bound native devices” passed (`packages/flutter_ai_communications_macos`). That code is **on `main`**, not uncommitted. Mac USB RMS=0 was not re-run on `e6b37b4`.
...
Suite gate in `example/integration_test/native_orchestration_test.dart`: `captureFrameCount > 1` **and** `recentRms > 0` within 8s. Silent frames fail. That gate stays.
...
| #18 diagnostics | Fake tests + Orchestration keys for Desired/Applied/Observed, frames, RMS, preferenceControlled, generation. | Suite already asserts these. Keep open until Chrome/desktop also expose them. |
...
| #21 web selection | Chrome: every available capture × render, `devicechange` promote/fallback, explicit blocks promote, streams survive reset, typed `sinkId` unsupported. | Policy unit tests exist. 20-cycle + 45-combination receipts at `10f9c2c`. **`devicechange` still skipped.** **Do not close.** |
...
| #26 Orchestration suite | Required matrix: physical iPhone, physical Android, Chrome. Capability-gated accessory rows recorded. Receipts with `nativeFailuresSkipped: false`. | Phone/tablet/iPhone/iPad 20-cycle rows exist. Chrome 20-cycle + combinations exist at `10f9c2c`; `devicechange` still skipped. Desktop is extra. **Do not close.** |
...
- `example/integration_test/native_orchestration_host.dart` (web stub)
- `example/integration_test/native_orchestration_host_io.dart` (io)
...
Observed emit is **on `main`**. Do not re-port it.
...
Receipt copy: suite prints `NATIVE_ORCHESTRATION_RECEIPT {json}` and writes under the device temp dir. Copy to `/tmp/flutter_ai_communications_receipts/<commit>-<platform>-<device>.json`.

## Working tree the next agent inherits

As of 2026-09-01 the working tree is **clean** on `main` (`e6b37b4`). Observed
emit, Orchestration rename, and camera graphs are on `main`, not uncommitted.

Historical (this handoff’s original uncommitted list) landed via PRs #31–#34.
Do not re-port Observed emit. Drive `native_orchestration_test.dart`. Workflow
file is `.agents/workflows/real-device-orchestration.md`.
...
- Not: interactive `flutter run` + flutter-skill taps. That path does not close hardware issues.
