  ...
  group('live WASAPI', () {
    test('catalog starts and emits non-zero capture', () async {
      ...
      expect(
        catalog,
        isNotEmpty,
        reason: 'WASAPI catalog was empty. logs=$logs',
      );
      expect(
    ...
    test('split capture/render stays on those Endpoints', () async {
      ...
      expect(
        ...
        reason:
            ...
            '${[for (final endpoint in catalog) '${endpoint.routeClass.name}:${endpoint.isCapture ? 'in' : 'out'}:${endpoint.name}'].join(' | ')}',
      );
      ...
      expect(
        ...
        NativeGraphStart.started,
        reason:
            'split start failed capture=${capture.id} render=${splitRender.id}',
      );
